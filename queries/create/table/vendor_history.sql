create table if not exists production.star."Vendor History" (
-- create or replace table production.star."Vendor History" (
    ...
    "Current Record" boolean comment 'Indicates if the record is the most recent and active entry for the vendor',
    "Effective Date" timestamp_ntz(9) comment 'Date when this version of vendor record took effect',
    "Expiration Date" timestamp_ntz(9) comment 'Date when this version of vendor record was replaced with a newer version',
    "Fax Number" varchar(50) comment 'Primary contact number for communicating via fax',
    "Key" number comment 'Unique identifier for this version of vendor',
    ...
    constraint "VendorHistoryPK" primary key ("Key")
)
    comment = 'History of changes to vendor records'
;