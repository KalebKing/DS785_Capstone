SELECT
    ...
    , V_BelegPos.Herk_BelegArt AS "V_BelegPos.Herk_BelegArt"
    , V_BelegPos.Herk_ReferenzNr AS "V_BelegPos.Herk_ReferenzNr"
    , V_BelegPos.Herk_PositionsNr AS "V_BelegPos.Herk_PositionsNr"
    ...
    , V_BelegKopf.BelegInfo AS "V_BelegKopf.BelegInfo"
    , V_BelegKopf.Kunde AS "V_BelegKopf.Kunde"
    , V_BelegKopf.Kurs AS "V_BelegKopf.Kurs"
    , V_BelegKopfAdr.AenderungDatum AS "V_BelegKopfAdr.AenderungDatum"
    , V_BelegKopfAdr.AenderungZeit AS "V_BelegKopfAdr.AenderungZeit"
    ...
    , V_BelegPos__l.Herk_BelegArt AS "V_BelegPos__l.Herk_BelegArt"
    , V_BelegPos__l.Herk_ReferenzNr AS "V_BelegPos__l.Herk_ReferenzNr"
    , V_BelegPos__l.PositionsNr AS "V_BelegPos__l.PositionsNr"
    , V_BelegKopf.BelegArt AS "V_BelegKopf.BelegArt"
    , V_BelegPos.LfdNr_SR AS "V_BelegPos.LfdNr_SR"
FROM
    PUB.V_BelegPos
    INNER JOIN PUB.V_BelegKopf
        ...
        AND V_BelegKopf.BelegArt in ('G', 'R')
        AND V_BelegKopf.Offen = 0
    ...
    LEFT JOIN PUB.V_BelegPos AS V_BelegPos__l
        ON V_BelegPos.Herk_BelegArt = V_BelegPos__l.BelegArt
        AND V_BelegPos.Herk_ReferenzNr = V_BelegPos__l.ReferenzNr
        AND V_BelegPos.Herk_PositionsNr = V_BelegPos__l.PositionsNr
    -- Exclude partial payments
    INNER JOIN PUB.S_Artikel
        ON V_BelegPos.Artikel = S_Artikel.Artikel
